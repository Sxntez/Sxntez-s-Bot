...
CHEST_COLORS = {
    "Common": 0x7f8c8d,      # grey
    "Uncommon": 0x2ecc71,    # light green
    "Rare": 0x3498db,        # blue
    "Epic": 0x9b59b6,        # purple
    "Legendary": 0xf22613,   # orange/red
    "Mythical": 0xFCF97E,    # white/gold
}
...
SHARD_CONVERSION_RECIPES = {
    "C": {"from": "F", "amount": 10},  # 10F -> 1C
    "B": {"from": "C", "amount": 10},  # 10C -> 1B
    "A": {"from": "B", "amount": 10},  # 10B -> 1A
    "S": {"from": "A", "amount": 10},  # 10A -> 1S
    "SS": {"from": "S", "amount": 10},  # 10S -> 1SS
}
...
async def try_spawn_from_activity(guild_id: int):
    ...
    # ⏱️ Not enough time passed (convert minutes to seconds)
    if now - last_spawn < SPAWN_EVERY_MINUTES * 60:
        return
...
BOT_OWNER_IDS = [515880067268739072, 432008257175945237, 717613530219806720, 266334464978845696, 558068224575537152, 894576628255055892, 1134120200728485931, 
                 1291291142767382561
...
async def spawn_in_guild(guild_id: int):
    ...
    if total_weight > 0 and random.random() <= min(total_weight, 1.0):
        ...
        chest_embed = discord.Embed(
            ...
            color=CHEST_COLORS.get(picked, 0x95a5a6)  # Default grey if rarity not found
        )
...
async def handle_trade_details(trade_id, user, channel):
    ...
    await channel.send(embed=instructions, delete_after=30)
    ...
    try:
        ...
        embed.add_field(
            ...
            value=(
                ...
                "  Filters: `r:rarity`, `t:tag`, `n:name`, `s:series`, `role:tank/dps/support`, `-wishlist @user`, `wl>>`, `o:time/rarity/r/power/wishlist/wl/w`\n"
                ...
                "`!tag <card_id> [card_id...] <emoji>` - Tag card(s) with an emoji\n"
                "`!taglatest <emoji>` - Tag your latest card (aliases: `!tl`, `!tagl`)\n"
        ...
        embed.add_field(
            ...
            value=(
                ...
                "`!convert <rarity> [amount]` - Convert shards (e.g., `!convert B 10`)\n"
...
async def forcespawn(ctx, *, arg: str = None):
    ...
    # --------------------------------
    # UPDATE PITY (forcespawn does NOT modify pity counter - only natural spawns do)
    # --------------------------------
...
async def pity(ctx):
    ...
    if not data:
        ...
        return
    await ctx.send(f"🔮 Spawns since SS: **{data['spawns_since_mythical']} / {PITY_LIMIT}**")
...
async def collection(ctx, *args):
    ...
    def parse_user_id(token: str):
        # Try parsing as mention first
        uid = parse_mention(token)
        if uid:
            return uid
    ...
    tag_filter = None
    role_filter = None
    wishlist_user_id = None  # Filter to cards on this user's wishlist
    show_role = False
    show_wishlist = False  # Show wishlist count at end of each row
    ...
    # Track which indices to skip (filter arguments)
    skip_indices = set()
    for i in range(len(tokens)):
        t_low = tokens[i].lower()
        if t_low == "-wishlist" and i + 1 < len(tokens):
            # Mark the next token (filter argument) to skip
            skip_indices.add(i + 1)

    for i, t in enumerate(tokens):
        if i in skip_indices:
            # This is a filter argument, preserve it in remaining
            remaining.append(t)
            continue
    ...
    while i < len(remaining):
        ...
        if arg_low.startswith("r:"):
            rarity_filter = normalize_rarity(arg_low[2:])
            if rarity_filter not in RARITIES:
                await ctx.send(f"❌ Invalid rarity `{arg_low[2:]}`.")
                return
            i += 1
            ...
            order_val = arg_low[2:].strip()
            # Normalize abbreviations
            if order_val in ("wl", "w"):
                order_by = "wishlist"
            elif order_val == "r":
                order_by = "rarity"
            else:
        ...
        elif arg_low == "-wishlist":
            # Next argument should be the user mention
            if i + 1 >= len(remaining):
                await ctx.send("❌ Missing user for `-wishlist` filter. Use `-wishlist @user` or `-wishlist userid`.")
                return
            
            wl_user_str = remaining[i + 1].strip()
            wl_uid = parse_mention(wl_user_str)
            
            if not wl_uid:
                # Try parsing as raw user ID
                try:
                    wl_uid = int(wl_user_str)
                except ValueError:
                    await ctx.send("❌ Invalid user format for `-wishlist` filter. Use `-wishlist @user` or `-wishlist userid`.")
                    return
            
            # Verify user exists
            try:
                wl_user = await bot.fetch_user(wl_uid)
                wishlist_user_id = wl_uid
            except discord.NotFound:
                await ctx.send(f"❌ User not found for `-wishlist` filter.")
                return
            i += 2  # Skip both -wishlist and the user argument
    ...
    # Add INNER JOIN for wishlist filter if needed
    if wishlist_user_id:
        query += """
        INNER JOIN wishlists wl
            ON wl.roster_character_id = c.roster_character_id
           AND wl.user_id = ?
        """
    ...
    params = []
    if wishlist_user_id:
        params.append(wishlist_user_id)
    ...
    def build_page_embed(page: int):
        ...
        for (gid, name, anime, rarity, power, role, wish_count, tag) in page_rows:
            ...
            wish_part = (
                f" • ♡ {wish_count}"
                if (order_by == "wishlist" and wish_count > 0) or (show_wishlist and wish_count > 0)
                else ""
                )
            role_line = (
                f"\n{ROLE_EMOJI.get(role,'🎭')}"
    ...
    while True:
        ...
        if emoji_str == "⏹️":
            ...
            page = total_pages - 1  # Go to last page
        elif emoji_str == "⏪":
...
@bot.command()
@is_bot_owner()
async def wipe(ctx, user_input: str, *, reason: str = "No reason provided"):
    import re
    ...
    if not uid:
        await ctx.send("❌ Invalid user format. Use `@user` or a user ID.")
        return
    
    member = ctx.guild.get_member(uid) if ctx.guild else None
    user_mention = member.mention if member else f"<@{uid}>"
    ...
    await ctx.send(f"🧹 Archived **{count}** cards for {user_mention}.")

@bot.command()
@is_bot_owner()
async def restore(ctx, user_input: str):
    import re
    ...
    count = claims_cur.rowcount
    await ctx.send(f"♻️ Restored **{count}** cards for {user_mention}.")
...
@is_bot_owner()
async def addshards_cmd(ctx, user_input: str, rarity: str, amount: int):
    import re
    ...
    await ctx.send(
        f"✅ {user_mention} now has **{new_amount}** "
...
async def tag(ctx, *args):
    if len(args) < 2:
        await ctx.send("❌ Usage: `!tag <card_id> [card_id...] <emoji>` (e.g., `!tag 001 002 003 :emoji:`)")
        return
    
    # Last argument is the emoji
    emoji = args[-1]
    card_inputs = args[:-1]
    ...
    for card_input in card_inputs:
        ...
        if global_id is None:
            invalid_codes.append(card_input)
        else:
            card_ids.append(global_id)
    
    if invalid_codes:
        await ctx.send(f"❌ Invalid card ID format(s): `{'`, `'.join(invalid_codes)}`. Use alphanumeric codes (minimum 3 characters).")
        return
    ...
    claims_cur.execute(f"""
        SELECT id, user_id FROM claimed_cards 
        WHERE id IN ({placeholders}) AND active = 1
    """, card_ids)
    
    rows = claims_cur.fetchall()
    card_owner_map = {row[0]: row[1] for row in rows}
    
    # Separate cards into valid and invalid
    valid_card_ids = []
    not_found = []
    not_owned = []
    
    for card_id in card_ids:
        if card_id not in card_owner_map:
            not_found.append(card_id)
        elif card_owner_map[card_id] != ctx.author.id:
            not_owned.append(card_id)
        else:
    ...
    if not valid_card_ids:
        error_msgs = []
        if not_found:
            codes = [int_to_code(cid) for cid in not_found]
            error_msgs.append(f"Cards not found: `{'`, `'.join(codes)}`")
        if not_owned:
            codes = [int_to_code(cid) for cid in not_owned]
            error_msgs.append(f"Cards you don't own: `{'`, `'.join(codes)}`")
        await ctx.send("❌ " + " | ".join(error_msgs))
        return
    ...
    for card_id in valid_card_ids:
        ...
        tagged_codes.append(int_to_code(card_id))
    
    claims_conn.commit()
    
    # Build response message
    if len(tagged_codes) == 1:
        await ctx.send(f"🏷️ Successfully tagged card `{tagged_codes[0]}` with {emoji}")
    else:
        await ctx.send(f"🏷️ Successfully tagged {len(tagged_codes)} cards (`{'`, `'.join(tagged_codes)}`) with {emoji}")
    
    # Report any issues with other cards
    if not_found or not_owned:
        warning_msgs = []
        if not_found:
            codes = [int_to_code(cid) for cid in not_found]
            warning_msgs.append(f"Not found: `{'`, `'.join(codes)}`")
        if not_owned:
            codes = [int_to_code(cid) for cid in not_owned]
            warning_msgs.append(f"Not owned: `{'`, `'.join(codes)}`")
        await ctx.send("⚠️ " + " | ".join(warning_msgs))

@bot.command()
async def tags(ctx, *args):
    # --- helper to parse <@123> or <@!123> or raw user ID ---
    def parse_mention(token: str):
        m = re.fullmatch(r"<@!?(\d+)>", token)
        return int(m.group(1)) if m else None
...
async def destroy(ctx, *card_inputs: str):
    ...
    card_lines = []
    for rarity in sorted(rarity_groups.keys(), key=lambda r: RARITY_ORDER.get(r, 0), reverse=True):
        names = rarity_groups[rarity]
        emoji = RARITY_EMOJI.get(rarity, "❔")
        card_lines.append(f"{emoji} **{rarity}**: {', '.join(f'**{name}**' for name in names)}")
...
@bot.command()
async def copy(ctx):
    """Copy card IDs from a collection message when replying to it"""
    # Check if message is a reply
    if not ctx.message.reference or not ctx.message.reference.message_id:
        await ctx.send("❌ Please reply to a collection message to copy card IDs.")
        return
    ...
    try:
        ...
        await ctx.send("❌ Couldn't find the message you're replying to.")
        return
    except discord.Forbidden:
        await ctx.send("❌ Don't have permission to access that message.")
        return
    
    # Check if it has an embed and is a collection embed
    if not replied_message.embeds:
        await ctx.send("❌ The replied message doesn't have an embed.")
        return
    
    embed = replied_message.embeds[0]
    
    # Check if it's a collection embed (title contains "Collection")
    if not embed.title or "Collection" not in embed.title:
        await ctx.send("❌ The replied message is not a collection message.")
        return
    
    # Extract card IDs from embed description
    # Card IDs are in backticks: `code`
    import re
    
    # Pattern to match backticked codes (at least 3 alphanumeric characters)
    # Card codes are in format: `{code}` where code is alphanumeric
    pattern = r'`([0-9a-z]{3,})`'
    matches = re.findall(pattern, embed.description or "")
    
    if not matches:
        await ctx.send("❌ No card IDs found in the collection message.")
        return
    
    # Format card IDs for easy copying (space-separated for easy copying on phone)
    card_ids_text = " ".join(matches)
    
    # If it's too long, split into multiple messages
    # Discord message limit is 2000 characters, leave room for formatting
    if len(card_ids_text) > 1900:
        ...
        for card_id in matches:
            # Add 1 for " " separator
            needed_length = len(card_id) + (1 if current_chunk else 0)
            if current_length + needed_length > 1900:
                chunks.append(" ".join(current_chunk))
                current_chunk = [card_id]
                current_length = len(card_id)
            else:
                current_chunk.append(card_id)
        ...
        for i, chunk in enumerate(chunks):
            await ctx.send(f"📋 **Card IDs** ({len(matches)} total):\n{chunk}")
    else:
        await ctx.send(f"📋 **Card IDs** ({len(matches)} total):\n{card_ids_text}")
...
async def recipes(ctx):
    ...
    # Page 1: Card Crafting
    def make_crafting_embed():
        embed = discord.Embed(
            title="🔨 Crafting Recipes",
            description="View crafting requirements for each rarity.",
            color=discord.Color.orange()
        )
        
        for rarity, recipe in CRAFT_RECIPES.items():
            cost_parts = []
            cost_parts.append(f"💰 {recipe['gold']:,} Gold")
            cost_parts.append(f"🧩 {recipe['shards']} {rarity} Shards")
            cost_text = " + ".join(cost_parts)
            
            embed.add_field(
                name=f"{RARITY_EMOJI[rarity]} {rarity} Crafting",
                value=(
                    f"`{cost_text}`\n\n"
        ...
        embed.set_footer(text="Page 1/2 • Use !craft <rarity> to craft a card")
        return embed
    
    # Page 2: Shard Conversions
    def make_conversion_embed():
        embed = discord.Embed(
            title="🔄 Shard Conversions",
            description="Convert lower rarity shards into higher rarity shards.",
            color=discord.Color.blue()
        )
        
        # Order: F->C, C->B, B->A, A->S, S->SS
        conversion_order = ["C", "B", "A", "S", "SS"]
        for target_rarity in conversion_order:
            if target_rarity in SHARD_CONVERSION_RECIPES:
                recipe = SHARD_CONVERSION_RECIPES[target_rarity]
                from_rarity = recipe["from"]
                amount = recipe["amount"]
                
                embed.add_field(
                    name=f"{RARITY_EMOJI[from_rarity]} → {RARITY_EMOJI[target_rarity]} Conversion",
                    value=(
                        f"`{amount} {from_rarity} Shards → 1 {target_rarity} Shard`\n\n"
                        f"Use `!convert {target_rarity.lower()}` to convert.\n"
                        f"{DIVIDER}"
                    ),
                    inline=False
                )
        
        embed.set_footer(text="Page 2/2 • Use !convert <rarity> to convert shards (e.g., !convert c)")
        return embed
    
    # Send first page
    msg = await ctx.send(embed=make_crafting_embed())
    ...
    page = 0  # 0 = crafting, 1 = conversions
    total_pages = 2
    ...
    while True:
        ...
        # Update embed based on page
        if page == 0:
            await msg.edit(embed=make_crafting_embed())
        else:
            await msg.edit(embed=make_conversion_embed())
...
@bot.command()
async def convert(ctx, rarity: str, amount: int = 1):
    rarity_input = rarity.strip().upper()
    user_id = ctx.author.id
    
    if rarity_input not in SHARD_CONVERSION_RECIPES:
        available = ', '.join(SHARD_CONVERSION_RECIPES.keys())
        await ctx.send(f"❌ Invalid rarity. Available for conversion: {available}")
        return
    ...
    recipe = SHARD_CONVERSION_RECIPES[rarity_input]
    from_rarity = recipe["from"]
    required_per_unit = recipe["amount"]
    target_rarity = rarity_input
    
    # Calculate total required
    total_required = required_per_unit * amount
    
    # Check if user has enough shards
    from_shards = get_shards(user_id, from_rarity)
    
    if from_shards < total_required:
        await ctx.send(f"❌ Not enough {from_rarity} shards. Required: {total_required} ({required_per_unit} × {amount}) | You have: {from_shards}")
        return
    
    # Perform conversion
    add_shards(user_id, from_rarity, -total_required)
    add_shards(user_id, target_rarity, amount)
    
    embed = discord.Embed(
        title="🔄 Shard Conversion Complete!",
        description=(
            f"Converted **{total_required} {from_rarity} Shards** → **{amount} {target_rarity} Shard{'s' if amount > 1 else ''}**\n\n"
            f"🧩 **{from_rarity} Shards Remaining:** {from_shards - total_required}\n"
            f"✨ **{target_rarity} Shards Now:** {get_shards(user_id, target_rarity)}"
        ),
        color=discord.Color.blue()
    )
    await ctx.send(embed=embed)
...
async def favorite(ctx, arg: str):
    ...
    # Parse card ID (alphanumeric codes only)
    card_id = parse_card_id(arg)
    if card_id is None:
        await ctx.send(f"❌ Invalid card ID format: `{arg}`. Use alphanumeric codes (minimum 3 characters) or `!fav clear`.")
        return
    ...
    if not row:
        card_code = int_to_code(card_id)
        await ctx.send(f"❌ You don't own an active card with ID `{card_code}`.")
        return

    set_favorite_card(ctx.author.id, card_id)
    card_code = int_to_code(card_id)
    await ctx.send(f"⭐ Favorite set! (Card `{card_code}` will show in your inventory overview.)")
...
async def open(ctx, chest_name: str):
    ...
    reward_embed = discord.Embed(
        ...
        description="\n".join(rewards),
        color=CHEST_COLORS.get(chest_name, 0x95a5a6)  # Default grey if rarity not found
    )
...
@is_bot_owner()
async def givekeys(ctx, user_input: str, rarity: str, amount: int):
    import re
    ...
    # Normalize rarity (capitalize first letter, rest lowercase)
    rarity = rarity.strip().capitalize()
    
    # Valid key rarities (quest/chest rarities, excluding Mythical)
    valid_key_rarities = {"Common", "Uncommon", "Rare", "Epic", "Legendary"}

    # Validate rarity
    if rarity not in valid_key_rarities:
        await ctx.send(
    ...
    # Get current keys
    current_keys = get_keys(uid).get(rarity, 0)
    ...
    await ctx.send(
        f"🗝️ {user_mention} now has **{new_amount}** **{rarity} Keys** "
        f"({sign}{delta})."
    )

@bot.command(name="givegold", aliases=["addgold", "ggold", "gg"])
@is_bot_owner()
async def givegold(ctx, user_input: str, amount: int):
    import re
    ...
    sign = "+" if delta > 0 else ""
    await ctx.send(
        f"💰 {user_mention} now has **{new_amount:,} Gold** ({sign}{delta:,})."
    )

@bot.command(name="giveshard", aliases=["addshard", "gshard", "gs"])
@is_bot_owner()
async def giveshard(ctx, user_input: str, rarity: str, amount: int):
    import re
    ...
    # Normalize card rarity (handles SS, S+, etc.)
    rarity = normalize_rarity(rarity)
    ...
    sign = "+" if delta > 0 else ""
    await ctx.send(
        f"🧩 {user_mention} now has **{new_amount} {rarity} Shards** ({sign}{delta})."
    )

@bot.command(name="givetoken", aliases=["addtoken", "gtoken", "gt"])
@is_bot_owner()
async def givetoken(ctx, user_input: str, token_type: str, amount: int):
    import re
    ...
    await ctx.send(
        f"{emoji} {user_mention} now has **{new_amount}** "
        f"**{token_type.capitalize()} Tokens** ({sign}{delta})."
    )

@bot.command(aliases=["trade","t"])
async def traderequest(ctx, user_input: str):
    ...
    claims_cur.execute("""
        ...
        AND status = 'pending'
    """, (ctx.author.id, uid, uid, ctx.author.id))
...
async def tradeadd(ctx, card_input: str):
    ...
    claims_cur.execute("""
        ...
        AND t.trade_id != ?
        AND (
            EXISTS (SELECT 1 FROM json_each(t.initiator_cards) WHERE CAST(value AS INTEGER) = ?) OR
            EXISTS (SELECT 1 FROM json_each(t.receiver_cards) WHERE CAST(value AS INTEGER) = ?)
        )
...
async def addimage(ctx, *, payload: str):
    ...
    !addimage Character ID | image_url
    """
    if "|" not in payload:
        await ctx.send("❌ Use: `!addimage Character Name/ID | image_url`")
        return

    character_input, image_url = [p.strip() for p in payload.split("|", 1)]
    ...
    if not char:
        await ctx.send(f"❌ Character not found. Use character name or ID (e.g., `!addimage 727 | image_url` or `!addimage Character Name | image_url`).")
        return
...
async def wishlist(ctx, user_input: str = None):
    import asyncio
    import re

    PER_PAGE = 20

    # Parse user input (mention or raw user ID)
    target = ctx.author
    if user_input:
        def parse_mention(token: str):
            m = re.fullmatch(r"<@!?(\d+)>", token)
...
@bot.command()
@is_bot_owner()
async def completequest(ctx, user_input: str):
    ...
    try:
        ...
        if not quest:
            await ctx.send(f"❌ {user_mention} doesn't have any active quest ready to collect!")
            return
